...
const emptyForm: ProductInput = { name: "", barcode: "", categoryId: "", brandId: "", unit: "", imageUrl: "", active: true };
...
export function ProductManager() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [nextProducts, nextCategories, nextBrands, nextUnits] = await Promise.all([productApi.listProducts(), productApi.listCategories(), productApi.listBrands(), productApi.listUnits()]);
      setProducts(nextProducts); setCategories(nextCategories); setBrands(nextBrands); setUnits(nextUnits);
  ...
  return <section className="workspace" id="productos">
    ...
    {(dialog === "create" || dialog === "edit") && <ProductForm dialog={dialog} form={form} errors={errors} categories={categories} brands={brands} units={units} pending={pending} errorSummary={errorSummary} onChange={changeForm} onClose={() => setDialog(null)} onSubmit={submit} />}
