...
5. Si no encontrás toda la cantidad de una línea, tocá **Reportar faltante** y elegí una resolución:
   - **Reemplazar por similar** permite buscar y seleccionar el sustituto. Conserva el precio para el cliente y cambia el producto reservado por el sustituto.
   - **Consultar al cliente** deja la línea faltante para consultar; se descuenta del pedido la parte no entregada.
   - **Quitar del pedido** elimina lo no entregado y también descuenta ese importe del pedido.
   En los tres casos podés registrar una nota opcional. Al confirmar, la línea queda resuelta y la app avanza a la siguiente pendiente.
6. Cuando **todas las líneas están resueltas**, se habilita **Finalizar picking**. Mientras haya líneas sin resolver, el botón está deshabilitado y se indica cuántas faltan.
...
`app/lib/picking-api.ts` es un adaptador temporal marcado. El contrato objetivo son los endpoints `/api/picking/tasks` del backend (PK-002 a PK-005): `?assignedTo=me`, `?status=PENDING`, `GET /:id`, `POST /:id/assign|start|complete`, `POST /:id/items/:itemId/pick` y `POST /:id/items/:itemId/shortage`. `reportShortage(taskId, itemId, resolution, substituteProductId?, note?)` registra `REPLACE_SIMILAR`, `CONTACT_ME` o `REMOVE_ITEM`; el sustituto es obligatorio sólo en el primer caso. `searchProducts(query)` usa el endpoint público `GET /api/products?q=` y degrada a una lista vacía ante un fallo de búsqueda. `pickItem(taskId, itemId, quantity, barcode?)` envía `barcode` sólo para un escaneo; una respuesta 400 de código no correspondiente se expone como `PickingApiError` con código estable `barcode_mismatch` y el mensaje para el picker. Sin `barcode`, la confirmación es una anulación manual auditada. `app/lib/picking-rules.js` (progreso, secuencia, `canCompleteTask`) refleja las reglas del backend y debe mantenerse alineado. Sin base URL usa un fixture en memoria, con algunos productos que tienen código, otros que ejercitan la confirmación manual y candidatos de sustitución buscables. No hay migraciones en este cambio (las del backend están en `superx_back`).
